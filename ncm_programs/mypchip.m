function [v,p] = mypchip(x,y,u)
%MYPCHIP  Textbook piecewise cubic Hermite interpolation.
%  v = mypchip(x,y,u) finds the shape-preserving piecewise cubic
%  interpolant P(x), with P(x(j)) = y(j), and returns v(k) = P(u(k)).
%  [v,p] = mypchip(x,y,u) also returns p(k) = P'(u(k)).
%  With only two input arguments, pp = mypchip(x,y) returns a
%  piecewise polynomial structure for use with ppval.
%
%  See PCHIP, SPLINETX.
...
%  Piecewise polynomial structure
   ...
   v = mkpp(x,[b' c' d(1:n-1)' y(1:n-1)'],1);
