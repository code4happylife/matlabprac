...
abutton = uicontrol('style','toggle','units','norm','pos',[.02 .26 .06 .06], ...
   'back','w','string','A', ...
   'callback','set(findobj(''string'',''B''),''value'',0)');
bbutton = uicontrol('style','toggle','units','norm','pos',[.02 .18 .06 .06], ...
   'back','w','string','B', ...
   'callback','set(findobj(''string'',''A''),''value'',0)');
...
while get(stop,'value') == 0
   ...
   if get(abutton,'value')
      xyz = reshape(A*w,15,3);
   elseif get(bbutton,'value')
      xyz = reshape(B*w,15,3);
   else
      f = reshape(F*w,15,3);
      m = reshape(M*w,15,3);
      xyz = (f+m)/2 + s(2)*(f-m)/2;
